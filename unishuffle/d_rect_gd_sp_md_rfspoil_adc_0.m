...
seq_note = strcat('sub',num2str(subject_num),'_random_',seq_mode_list{seq_mode},'_rect_rfsp_qua',num2str(rf_spoiling_increment), '_adc_ph');
...
if use_rfspoiler
 % Spoiling increment in degrees
    ...
    for indLine = 1:iLine
        ...
        rf_spoil_list{indLine} =  rf;
        rf_spoil_list{indLine} .phaseOffset = rf_spoiling_phase_rad;
        rf_spoil_list{indLine} .phaseOffset;
        clear rf;
...
if add_rfdelay
    rf_delay = mr.makeDelay(rf_spoil_list{1}.deadTime*rfdelay_ratio);
end
%%
plot_rfpulses = 0;

if plot_rfpulses
    disp(angle(rf_spoil_list{1}.signal(1)));

    figure;
    set(gcf, 'Color', 'w'); 
    subplot(1,3,1);
    plot(rf_spoil_list{3}.phaseOffset, 'r-', 'LineWidth', 2); % Phase of processed RF
    % xline(tao, '--k', 'LineWidth', 1.5);
    % legend('Original b1 phase', 'Processed RF1 phase');
    title('Phase Comparison for RF1 and RF2');
    % ylim([-pi, pi]);
...
% TE_kernel
if add_rfdelay
    te_kernel = (mr.calcDuration(rf_spoil_list{1})-rf_spoil_list{1}.delay)/2  + mr.calcDuration(rf_delay) + mr.calcDuration(gzPre) + gz.riseTime+gz.flatTime/2;
else
    te_kernel = (mr.calcDuration(rf_spoil_list{1})-rf_spoil_list{1}.delay)/2  + mr.calcDuration(gxPre) + mr.calcDuration(gx)/2;
end
...
% TR_kernel
if add_rfdelay
    tr_kernel = mr.calcDuration(rf_spoil_list{1}) + mr.calcDuration(rf_delay) + delay_te + mr.calcDuration(gz_1)+ mr.calcDuration(gz_2);
else
    tr_kernel = mr.calcDuration(rf_spoil_list{1}) + delay_te + mr.calcDuration(gz_1)+ mr.calcDuration(gz_2);
end
...
traj_design = 3;    % Trajectory design: 0 = original, 1 = pole-to-pole, 2 = continuous
...
for indLine = 1:iLine
        ...
        %%%++++++++++++++++++++++++++
        seq.addBlock(rf_spoil_list{indLine})
...
make_gif = false;
k_trj = kspace_traj;
k_trj = reshape(k_trj, [3, 480, 22, round(iLine/nSeg)]);
%%
if plot_shot
    %plot k-spaces
    % figure;
