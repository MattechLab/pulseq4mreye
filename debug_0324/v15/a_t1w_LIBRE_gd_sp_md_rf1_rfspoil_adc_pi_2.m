...
grad_mode = 'Fast';
checking = true;
seq_note = 'b_v15_prescan_t1w_LIBRE_gd_sp_md_rf1_rfspoil_adc_pi_2';
add_rfdelay = true;
rfdelay_ratio = 1;
do_compile = 1;
seqfolder = "/Users/cag/Documents/forclone/pulseq4mreye/debug_0324/seqs_v15";
methodrf = 1; 
use_rfspoiler = 1;
...
if use_rfspoiler
    ...
    for indLine = 1:iLine
        ...
        [rf1] = mr.makeArbitraryRf([b1] , alpha*pi/180, 'use','excitation', sys);
        [rf2] = mr.makeArbitraryRf([b2] , alpha*pi/180, 'use','excitation', sys);
...
for idx_rf = 10:20
    disp('--------')
    disp(angle(rf1_spoil_list{idx_rf}.signal(end)));
    disp(angle(rf2_spoil_list{idx_rf}.signal(1)));
    figure;
    set(gcf, 'Color', 'w'); 
    subplot(1,3,1);
    plot(angle(rf1_spoil_list{idx_rf}.signal), 'r-', 'LineWidth', 2); % Phase of processed RF
    ...
    subplot(1,3,2);
    plot(angle(rf2_spoil_list{idx_rf}.signal), 'r-', 'LineWidth', 2);
    xline(tao, '--k', 'LineWidth', 1.5);
    ...
    if angle(rf1_spoil_list{idx_rf}.signal(end))>angle(rf2_spoil_list{idx_rf}.signal(1))
        angle_diff = angle((rf1_spoil_list{idx_rf}.signal(end)))-(angle(rf2_spoil_list{idx_rf}.signal(1)));  
    else
        angle_diff = (angle(rf1_spoil_list{idx_rf}.signal(end))+pi)+(pi-angle(rf2_spoil_list{idx_rf}.signal(1)));
    end
    
    angle_array = angle_diff*ones(1100,1);
    plot(angle_array, 'b'); hold on;
    
    xline(tao, '--k', 'LineWidth', 1.5);
    % legend('Original b2-b1 phase', 'Processed RF2 phase-RF1 phase');
    title(['RF2-RF1 with idx--', num2str(idx_rf)]);
    ylim([-4.2, 4.2]);
...
% TR_kernel
if add_rfdelay
    tr_kernel = mr.calcDuration(rf1_spoil_list{1}) + mr.calcDuration(rf2_spoil_list{1}) + mr.calcDuration(rf2_delay) + delay_te + mr.calcDuration(gz_1)+ mr.calcDuration(gz_2);
else
...
% Max. Gradient: 1098326 Hz/m == 25.80 mT/m
% Max. Gradient: 1098385 Hz/m == 25.80 mT/m
% Max. Gradient: 1098901 Hz/m == 25.81 mT/m
% Max. Slew Rate: 4.99239e+09 Hz/m/s == 117.26 T/m/s
% Max. Slew Rate: 4.99266e+09 Hz/m/s == 117.26 T/m/s
% Max. Slew Rate: 4.995e+09 Hz/m/s == 117.32 T/m/s
% Max. Absolute Gradient: 1098902 Hz/m == 25.81 mT/m
% Max. Absolute Slew Rate: 4.99501e+09 Hz/m/s == 117.32 T/m/s
