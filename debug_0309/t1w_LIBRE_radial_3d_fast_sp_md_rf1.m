...
grad_mode = 'Fast';
checking = true;
seq_note = 't1w_LIBRE_radial_3d_fast_sp_md_2rf';
add_rfdelay = true;
rfdelay_ratio = 1;
do_compile = false;
methodrf = 1; 
...
figure;
subplot(3,1,1);
plot(angle(b1), 'b-'); hold on;
plot(angle(rf1.signal), 'r--'); % Phase of processed RF
legend('Original b1 phase', 'Processed RF1 phase');
title('Phase Comparison for RF1');

subplot(3,1,2);
plot(angle(b2), 'b'); hold on;
plot(angle(rf2.signal), 'r--');
legend('Original b2 phase', 'Processed RF2 phase');
title('Phase Comparison for RF2');

subplot(3,1,3);
plot(angle(b2)-angle(b1), 'b'); hold on;
plot(angle(rf2.signal)-angle(rf1.signal), 'r--');
...
% TR_kernel
if add_rfdelay
    tr_kernel = mr.calcDuration(rf1) + mr.calcDuration(rf2) + mr.calcDuration(rf2_delay) + delay_te + mr.calcDuration(gz_1)+ mr.calcDuration(gz_2);
else
