...
%this file use a different way to add phase, instead of add phase on b1 and
%b2, add the phase afterward directly in .phaseOffset.
% CHUV Prisma
...
for seq_mode = seq_mode_array
    ...
    % seq_mode = 1;%1: pre 2: main 3: debug
    add_rfdelay = true;
    rfdelay_ratio = 1;
    do_compile = 0;
    seqfolder = "/Users/cag/Documents/forclone/pulseq4mreye/mreye2p0_0424/";
    use_rfspoiler = 1;
    
    %%%%
    seq_mode_list = {'pre', 'main', 'debug'};
    seq_note = strcat('sub',num2str(subject_num),'_',seq_mode_list{seq_mode},'_libre_t1w_',note_suffix);
    ...
    % In this case there will be overlapping at the k-space center but with one more sample
    ...
    [rf1] = mr.makeArbitraryRf([b1] , alpha*pi/180, sys, 'use', 'excitation');
    [rf2] = mr.makeArbitraryRf([b2] , alpha*pi/180, sys, 'use', 'excitation');
    ...
    if plot_rfpulses
        disp(angle(rf1.signal(1)));
        disp(angle(b2(1)));
        
        figure;
        set(gcf, 'Color', 'w'); 
        subplot(1,3,1);
        plot(angle(rf1.signal), 'r-', 'LineWidth', 2); % Phase of processed RF
        ...
        subplot(1,3,2);
        plot(angle(rf2.signal), 'r-', 'LineWidth', 2);
        xline(tao, '--k', 'LineWidth', 1.5);
        ...
        subplot(1,3,3);
        plot(angle(rf2.signal)-angle(rf1.signal), 'b'); hold on;
    ...
    % TE_kernel
    if add_rfdelay
        te_kernel = mr.calcDuration(rf2)  + mr.calcDuration(rf2_delay) + mr.calcDuration(gzPre) + gz.riseTime+gz.flatTime/2;
    else
    ...
    % TR_kernel
    if add_rfdelay
        tr_kernel = mr.calcDuration(rf1) + mr.calcDuration(rf1_delay) + mr.calcDuration(rf2) + mr.calcDuration(rf2_delay) + delay_te + mr.calcDuration(gz_1)+ mr.calcDuration(gz_2);
    else
        tr_kernel = mr.calcDuration(rf1) + mr.calcDuration(rf2) + delay_te + mr.calcDuration(gz_1)+ mr.calcDuration(gz_2);
    end
    ...
    disp(['traj --nShot: ',num2str(nShot), '--nSeg: ', num2str(nSeg)])
    switch traj_design
        case 0
            disp('Original phyllotaxis!!')
    ...
    for indLine = 1:iLine
            ...
            %%%++++++++++++++++++++++++++
            rf1.phaseOffset = rf_spoiling_phase_rad{indLine};
            seq.addBlock(rf1)
            if add_rfdelay
                seq.addBlock(rf1_delay);
            end
    
            rf2.phaseOffset = rf_spoiling_phase_rad{indLine};
            seq.addBlock(rf2)
            if add_rfdelay
            ...
            adc.delay         = adc.delay+mr.calcDuration(gzPre); 
            trigger_array = [1,1001,2001];
            if use_trigger && ismember( indLine, trigger_array)
                % Create trigger wait block using Pulseq's event library
                    trigger_delay = adc.delay+mr.calcDuration(gzPre)-2*seq.gradRasterTime;
                    extTrigger = mr.makeDigitalOutputPulse('ext1', 'delay', trigger_delay,'duration', 5e-6); 
                    seq.addBlock(gx1,gy1,gz1,adc, extTrigger);
                    disp('add trigger here!!')
                    disp(['Line Index:', num2str(indLine)])
            else
